...
- `lam --completions <shell>` prints a static shell completion script for
  `bash`, `zsh`, or `fish`. The script completes flag names, their fixed
  enum values (`--to`, `--format`, `--flatten-cells`, `--completions`),
  and file paths. It is generated by introspecting the CLI's `ArgParser`,
  so the completable flags and values cannot drift from the real
